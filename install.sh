...
#
# Idempotent: re-running repairs and continues. Safe to run after failures.
...
# migrations ----------------------------------------------------------------
# Apply schema migrations (0001..0004) to a fresh or partially-migrated DB.
# The runner is idempotent: already-applied migrations are skipped by
# file-hash checkpoint, so re-running the installer is safe.
# NOTE: --entrypoint replaces the etl service's `python -m state.etl`
# entrypoint; without it these args would be parsed by state.etl instead.
log "Running database migrations..."
docker compose run --rm --entrypoint python etl -m core.migrations.migrate --direction=up \
  || die "Migration runner failed — apply manually: docker compose run --rm --entrypoint python etl -m core.migrations.migrate --direction=up (needs DATABASE_URL or .env)"
...
CRITICAL RULES:
- Always call get_server_docs first to load data conventions before any query.
- Names are stored LAST-FIRST (e.g. payee_naml='Daly', namf='Michael Gomez').
- On rcpt_cd, cmte_id identifies the DONOR's committee, not the recipient. Always scope queries through filer_filings_cd, not cmte_id.
- 24-hour expenditure reports (s496_cd) have NO payee names — only a description. Vendor data from expn_cd is a lower bound; use rapid_expense_vendors to recover payees.
- Never trust a large total without spot-checking the raw rows for that filing_id — a filing may contain many different donors, not just the one you're interested in.
- If you join through filer_filings_cd to resolve a committee name, verify the result by checking the raw donor names on the individual rows for that filing_id.
